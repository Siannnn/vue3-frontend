...
import { reqSPUList, reqSkuList, reqRemoveSpu } from "@/api/product/spu";
...
//查看sku列表
let skuArr = ref<SkuData[]>([]); //存储查看的sku数据
...
let show = ref(false); //控制dialogue展示
//删除spu
const deleteSpu = (spuId: number) => {
  reqRemoveSpu(spuId).then((res) => {
    if (res.code == 200) {
      //删除成功
      getSPUList(
